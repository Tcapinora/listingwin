...
import { clampOpenHomeOverlay } from "@/components/DraggableOpenHome";
...
const controls: Array<{
  ...
  { key: "x", label: "Left / right", icon: "move", min: 0, max: 93 },
  { key: "y", label: "Up / down", icon: "move", min: 0, max: 88 },
  { key: "width", label: "Size", icon: "size", min: 7, max: 80 },
];

export function OpenHomePlacementControls({
  overlay,
  onChange,
}: {
      ...
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-blue-900/70">
            Drag the buyers on the photo, or fine tune position and size here.
          </p>
        </div>
        <button
          type="button"
          ...
          aria-label="Reset open home placement"
          title="Reset open home placement"
        >
          <RotateCcw size={15} />
        </button>
      </div>
